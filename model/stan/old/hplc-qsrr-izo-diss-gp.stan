...
transformed parameters {
  ...
  param_gp = miu'+param'*L_K;
     
  for (i in 1 : nAnalytes) {
    logkx[start[i]:end[i]] = funlogki(param_gp[1,i],param_gp[2,i], S2Hat, fi[start[i]:end[i]]);
  }
...
model {
  ...
  for (i in 1 : nAnalytes) {
  param[i] ~ multi_normal([0,0], Omega);
  }
