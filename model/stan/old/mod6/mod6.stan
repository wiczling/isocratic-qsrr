...
transformed parameters {
  ...
  matrix[nAnalytes_uncorr, 2] miu_uncorr;
  vector[nObs] logkx;
...
model {
  ...
  L_K_g =cholesky_decompose(identity_matrix(n_g)*(nu-2+1));
  target += matrix_normal_lpdf(to_vector(param_g) | to_vector(miu_g), L_K_g, L_Omega_W);
}

  matrix[nAnalytes_uncorr, nAnalytes_uncorr] L_K_x;
  L_K_x =cholesky_decompose(identity_matrix(nAnalytes_uncorr)*(nu-2+1));
  target += matrix_normal_lpdf(to_vector(param_uncorr) | to_vector(miu_uncorr), L_K_x, L_Omega_W);
