...
// Simplified version of the code that uses pKa as a predictor
// one sigma
// pKas and alphas without etas
// cumulative_sum, log_softmax version
...
functions {
 ...
 real funlogki(vector logkw, vector S1, real S2, vector pKaw, vector alpha, int R, real fi) {
    ...
    loglambda = log_softmax(cpHmpKa[1:(R+1)]);
    
    if (R == 0) {
      lnki = logkix[1];
    } else if (R>0)  {
      lnki = log_sum_exp(loglambda + logkix[1:(R+1)]);
    } 
...
transformed parameters {
  ...
  for (d in 1 : nA0) {
     logkwx[idxA0[d], 1] += dlogkwHat[1];
     logkwx[idxA0[d], 2] += dlogkwHat[1];
     logkwx[idxA0[d], 3] += dlogkwHat[1];
     logkwx[idxA0[d], 4] += dlogkwHat[1]; 
   }
    for (d in 1 : nB0) {
     logkwx[idxB0[d], 1] += dlogkwHat[2];
     logkwx[idxB0[d], 2] += dlogkwHat[2];
     logkwx[idxB0[d], 3] += dlogkwHat[2];
     logkwx[idxB0[d], 4] += dlogkwHat[2]; 
   }
   
  for (d in 1 : nGroupsA) {
    if (idxGroupsA[d,2]==1) {
      logkwx[idxGroupsA[d,1], 2] += dlogkwHat[1];
      logkwx[idxGroupsA[d,1], 3] += dlogkwHat[1];
      logkwx[idxGroupsA[d,1], 4] += dlogkwHat[1];
   }
   if (idxGroupsA[d,2]==2) {
      logkwx[idxGroupsA[d,1], 3] += dlogkwHat[1];
      logkwx[idxGroupsA[d,1], 4] += dlogkwHat[1];
  ...
  for (d in 1 : nGroupsB) {
      if (idxGroupsB[d,2]==3) {
      logkwx[idxGroupsB[d,1], 3] += dlogkwHat[2];
      logkwx[idxGroupsB[d,1], 2] += dlogkwHat[2];
      logkwx[idxGroupsB[d,1], 1] += dlogkwHat[2];
   }
   if (idxGroupsB[d,2]==2) {
      logkwx[idxGroupsB[d,1], 2] += dlogkwHat[2];
      logkwx[idxGroupsB[d,1], 1] += dlogkwHat[2];
   ...
   for (d in 1 : nA0) {
     S1x[idxA0[d], 1] += dS1Hat[1];
     S1x[idxA0[d], 2] += dS1Hat[1];
     S1x[idxA0[d], 3] += dS1Hat[1];
     S1x[idxA0[d], 4] += dS1Hat[1]; 
   }
   
   for (d in 1 : nB0) {
     S1x[idxB0[d], 1] += dS1Hat[2];
     S1x[idxB0[d], 2] += dS1Hat[2];
     S1x[idxB0[d], 3] += dS1Hat[2];
     S1x[idxB0[d], 4] += dS1Hat[2]; 
   }
   
  for (d in 1 : nGroupsA) {
    if (idxGroupsA[d,2]==1) {
      S1x[idxGroupsA[d,1], 2] += dS1Hat[1];
      S1x[idxGroupsA[d,1], 3] += dS1Hat[1];
      S1x[idxGroupsA[d,1], 4] += dS1Hat[1];
   }
   if (idxGroupsA[d,2]==2) {
      S1x[idxGroupsA[d,1], 3] += dS1Hat[1];
      S1x[idxGroupsA[d,1], 4] += dS1Hat[1];
  ...
  for (d in 1 : nGroupsB) {
      if (idxGroupsB[d,2]==3) {
      S1x[idxGroupsB[d,1], 3] += dS1Hat[2];
      S1x[idxGroupsB[d,1], 2] += dS1Hat[2];
      S1x[idxGroupsB[d,1], 1] += dS1Hat[2];
   }
   if (idxGroupsB[d,2]==2) {
      S1x[idxGroupsB[d,1], 2] += dS1Hat[2];
      S1x[idxGroupsB[d,1], 1] += dS1Hat[2];
