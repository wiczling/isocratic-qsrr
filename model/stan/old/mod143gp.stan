...
data {
  ...
  matrix[nAnalytes,nAnalytes] distance; //  
...
transformed parameters {
  ...
  for (d in 1 : nA0) {
   ...
   for (j in (i + 1):nAnalytes) {
      K[i, j] = square(alpha)*exp(-0.5/square(rho_gp)*distance[i,j]);
      K[j, i] = K[i, j];
     }
   }
  ...
  //  param = miu + diag_pre_multiply(omega, rhoc * eta')';
