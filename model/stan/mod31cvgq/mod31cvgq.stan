...
data {
  ...
  matrix[nAnalytes,nAnalytes] similarity_s;
...
model {
  ...
  matrix[nAnalytes, nAnalytes] L_K;
   ...
   L_K =cholesky_decompose(similarity_s*alpha + (1.0-alpha)*identity_matrix(nAnalytes));
...
generated quantities {
  ...
  
 {
  ...
  L_K_gq =cholesky_decompose(similarity_s*alpha + (1.0-alpha)*identity_matrix(nAnalytes));
  ...
  sparam_pop= miu + L_K_gq * eta_pop * L_Omega';

  matrix[nAnalytes, 2] temp = mdivide_left_tri_low(L_K_gq, seta_ind);
