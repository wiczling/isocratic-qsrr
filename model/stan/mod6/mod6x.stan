...
transformed parameters {
  ...
  matrix[nAnalytes_corr, nAnalytes_corr] L_K; // Cholesky factor of K
  {
    matrix[nAnalytes_corr, nAnalytes_corr] K = gp_exp_quad_cov(distance_s, 1.0, rhogp);
    for (n in 1:nAnalytes_corr) {
      K[n, n] = K[n, n] + 0.01; // Add jitter for numerical stability
    }
  ...
  lprior += normal_lpdf(beta[1] | 0.7, 0.125);
  lprior += normal_lpdf(beta[2] | 0.5, 0.5);
  ...
  lprior += normal_lpdf(sigma | 0, 0.05);
   ...
   real lposterior = 0;
  lposterior += kronecker_normal_lpdf(to_vector(param_corr) | to_vector(miu[idx_corr, 1:2]), L_K, L_Omega);

  // Likelihood for uncorrelated parameters
  for (i in 1:nAnalytes_uncorr) {
  lposterior += multi_normal_lpdf(param_uncorr[i] | miu[idx_uncorr[i], 1:2], Omega);
  }
