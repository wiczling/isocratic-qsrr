...
model {
  ...
  K = gp_exp_quad_cov(distance_s, 1.0, rhogp) + 
      diag_matrix(rep_vector(0.01, nAnalytes_corr));
  L_K = cholesky_decompose(K);
  }

  target += kronecker_normal_lpdf(to_vector(param_corr) | to_vector(miu_corr), L_K, L_Omega_W);
