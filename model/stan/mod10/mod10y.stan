...
data {
  ...
  matrix[nAnalytes,nAnalytes] mask_x;
  int max_n_g;
}

transformed data {
  vector[nAnalytes] logP_centered = logPobs - 2.5;
  matrix[nAnalytes_corr,nAnalytes_corr] mask_s;
  
  mask_s = mask_x[idx_corr,idx_corr];

  array[mGroup] int n_corr_per_group;
  array[mGroup, nAnalytes_corr] int idx_corr_group;

  // Assign correlation indices to groups
  for (g in 1:mGroup) {
    int count = 0;
...
parameters {
  ...
  array[mGroup] cholesky_factor_corr[max_n_g] L_K_group;
}
...
model {
  ...
  matrix[n_g, 2] miu_g;
  matrix[n_g, n_g] Precision;
  matrix[n_g, n_g] L_K_g;
  matrix[n_g, n_g] mask_g;
  ...
  for (i in 1:n_g) {
   ...
   // Use the stored Cholesky factor
  L_K_g = L_K_group[g][1:n_g, 1:n_g];
  Precision = chol2inv(L_K_g);
  mask_g = mask_s[idx_g,idx_g];
  
  target += lkj_corr_cholesky_lpdf(L_K_g| 2.0);
  
  for (i in 2:n_g) {
  for (j in 1:(i - 1)) {
    if (mask_g[i, j] == 0) {
      target += normal_lpdf(Precision[i, j] | 0, 0.01);
    }
  }
}
