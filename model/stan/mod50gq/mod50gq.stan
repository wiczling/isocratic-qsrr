...
model {
  ...
  for (i in 1:n_g) {
   ...
   L_K_g =cholesky_decompose(2*alpha*(similarity_s[idx_g,idx_g]-0.5) + (1.0-alpha)*identity_matrix(n_g));
...
generated quantities {
  ...
  
 {
  ...
  L_K_g_gq =cholesky_decompose(2*alpha*(similarity_s[idx_g,idx_g]-0.5) + (1.0-alpha)*identity_matrix(n_g));
