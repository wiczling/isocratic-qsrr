...
transformed parameters {
  ...
  {
  for (i in 1 : nAnalytes) {
    miulogkw[i, :] = (logkwHat + beta[1] * logPobs[i] + fgrp[i,1:K] * pilogkw)*[1,1,1,1]';
    miuS1[i, :] = (S1Hat + beta[2] * logPobs[i] + fgrp[i,1:K] * piS1)*[1,1,1,1]';
  }
  
  for (d in 1 : nA0) {
     miulogkw[idxA0[d], 1:4] += dlogkwHat + kappa[1]*etadlogkwA0[d];
     miuS1[idxA0[d], 1:4] += dS1Hat + kappa[2]*etadS1A0[d];
     }
  for (d in 1 : nB0) {
     miulogkw[idxB0[d], 1:4] += dlogkwHat + kappa[1]*etadlogkwB0[d];
     miuS1[idxB0[d], 1:4] += dS1Hat + kappa[2]*etadS1B0[d];
     }
   
  for (d in 1 : nGroupsA) {
      miulogkw[idxGroupsA[d,1], (idxGroupsA[d,2]+1) : 4] += dlogkwHat + kappa[1]*etadlogkwA[d];
      miuS1[idxGroupsA[d,1], (idxGroupsA[d,2]+1) : 4] += dS1Hat + kappa[2]*etadS1A[d];
      }
  
  for (d in 1 : nGroupsB) {
      miulogkw[idxGroupsB[d,1], 1:idxGroupsB[d,2]] += dlogkwHat + kappa[1]*etadlogkwB[d];
      miuS1[idxGroupsB[d,1], 1:idxGroupsB[d,2]] += dS1Hat + kappa[2]*etadS1B[d];
      }
...
generated quantities {
  ...
  vector[nA0] etadlogkwA0_pop;
  vector[nA0] etadS1A0_pop;
  vector[nB0] etadlogkwB0_pop;
  vector[nB0] etadS1B0_pop;
  vector[nGroupsA] etadlogkwA_pop;
  vector[nGroupsA] etadS1A_pop;
  vector[nGroupsB] etadlogkwB_pop;
  vector[nGroupsB] etadS1B_pop;
  ...
  array[nAnalytes] vector[maxR + 1] miulogkw_pop;
  array[nAnalytes] vector[maxR + 1] miuS1_pop;
  array[nSteps] vector[2] miu_pop;
  ...
  {
  for (i in 1 : nAnalytes) {
    miulogkw_pop[i, :] = (logkwHat + beta[1] * logPobs[i] + fgrp[i,1:K] * pilogkw)*[1,1,1,1]';
    miuS1_pop[i, :] = (S1Hat + beta[2] * logPobs[i] + fgrp[i,1:K] * piS1)*[1,1,1,1]';
  }
  
  for (d in 1 : nA0) {
     etadlogkwA0_pop[d] = normal_rng(0,1);
     etadS1A0_pop[d] = normal_rng(0,1);
     miulogkw_pop[idxA0[d], 1:4] += dlogkwHat + kappa[1]*etadlogkwA0_pop[d];
     miuS1_pop[idxA0[d], 1:4] += dS1Hat + kappa[2]*etadS1A0_pop[d];
     }
  for (d in 1 : nB0) {
     etadlogkwB0_pop[d] = normal_rng(0,1);
     etadS1B0_pop[d] = normal_rng(0,1);
     miulogkw_pop[idxB0[d], 1:4] += dlogkwHat + kappa[1]*etadlogkwB0_pop[d];
     miuS1_pop[idxB0[d], 1:4] += dS1Hat + kappa[2]*etadS1B0_pop[d];
     }
   
  for (d in 1 : nGroupsA) {
      etadlogkwA_pop[d]= normal_rng(0,1);
      etadS1A_pop[d] = normal_rng(0,1);
      miulogkw_pop[idxGroupsA[d,1], (idxGroupsA[d,2]+1) : 4] += dlogkwHat + kappa[1]*etadlogkwA_pop[d];
      miuS1_pop[idxGroupsA[d,1], (idxGroupsA[d,2]+1) : 4] += dS1Hat + kappa[2]*etadS1A_pop[d];
      }
  
  for (d in 1 : nGroupsB) {
      etadlogkwB_pop[d]= normal_rng(0,1);
      etadS1B_pop[d] = normal_rng(0,1);
      miulogkw_pop[idxGroupsB[d,1], 1:idxGroupsB[d,2]] += dlogkwHat + kappa[1]*etadlogkwB_pop[d];
      miuS1_pop[idxGroupsB[d,1], 1:idxGroupsB[d,2]] += dS1Hat + kappa[2]*etadS1B_pop[d];
      }
   
// convert to long format
   for (nc in 1 : nSteps) { 
   miu_pop[nc,1]  =  miulogkw_pop[idxch[nc,1],idxch[nc,2]];
   miu_pop[nc,2]  =  miuS1_pop[idxch[nc,1],idxch[nc,2]];
  }
  }
  
  
  for (nc in 1 : nSteps) {
    sparam_pop[nc] = multi_normal_rng(miu_pop[nc,1:2],Omega);
