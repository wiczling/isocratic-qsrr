...
transformed data {
  ...
  matrix[nAnalytes,nAnalytes] L_K=identity_matrix(nAnalytes);
}
...
generated quantities {
  ...
  sparam_pop= miu + (L_K * eta_pop * L_V');
   ...
   log_lik1 = matrix_normal_lpdf(to_vector(param) | to_vector(miu), L_K, L_V);
   log_lik2 = 0;
   for (i in 1:nAnalytes) log_lik2 += multi_normal_lpdf(param[i] | miu[i], Omega);
  }
