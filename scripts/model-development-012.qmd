---
title: "QSRR revisited. Model 010. GPU"
author:
  - name: "Paweł Wiczling*"
    affiliations:
...
for (g in 1:nGroups) {
  ...
  idx_corr_group[[g]]<-idx_corelatted[which(group==ugroup[g])]
  
  logkobs_group[[g]]<-data$logk[analyte %in% idx_corr_group[[g]]]
  fi_group[[g]]<-data$fi[analyte %in% idx_corr_group[[g]]]
  ids_temp <- data$ID[analyte %in% idx_corr_group[[g]]]
  nObs_group[[g]] <- length(logkobs_group[[g]])
  start_group[[g]] <- (1:nObs_group[[g]])[!duplicated(ids_temp)]
  end_group[[g]] <- c(start_group[[g]][-1] - 1, nObs_group[[g]])
}

# uncorrelated
g = nGroups+1
  logkobs_group[[g]]<-data$logk[analyte %in% idx_uncorelatted]
  fi_group[[g]]<-data$fi[analyte %in% idx_uncorelatted]
  ids_temp <- data$ID[analyte %in% idx_uncorelatted]
  nObs_group[[g]] <- length(logkobs_group[[g]])
  start_group[[g]] <- (1:nObs_group[[g]])[!duplicated(ids_temp)]
  end_group[[g]] <- c(start_group[[g]][-1] - 1, nObs_group[[g]])
  nAnalyte_subset_groups[[g]]=length(unique(ids_temp))
  idx_corr_group[[g]]<-idx_uncorelatted
...
# Add group-specific nD, point_mu_lower, and point_sd_lower with _group suffix
  for (g in 1:(nGroups+1)) {
    datastruct[[paste0("nObs_", g)]] <- nObs_group[[g]]
    datastruct[[paste0("logkobs_", g)]] <- logkobs_group[[g]]
    datastruct[[paste0("fi_", g)]] <- fi_group[[g]]
    datastruct[[paste0("starta_", g)]] <- start_group[[g]]
    datastruct[[paste0("enda_", g)]] <- end_group[[g]]
    datastruct[[paste0("nAnalyte_subset_", g)]] <- nAnalyte_subset_groups[[g]]
    datastruct[[paste0("idx_corr_", g)]] <- idx_corr_group[[g]]
  }
...
init <- function() {
  init_list <- list(
    ...
    param_11 = cbind(init_aprox$logkw[idx_uncorelatted],    init_aprox$S1[idx_uncorelatted]),
    nu = 20,
    pilogkw = rep(0, nK),
    piS1 = rep(0, nK),
  ...
  for (g in 1:nGroups) {
    ...
    init_list[[paste0("param_corr_", g)]] <-cbind(init_aprox$logkw[idx_corr_group[[g]]],                  init_aprox$S1[idx_corr_group[[g]]])
  }
  init_list
}
```
...
n <- nGroups+1  # Change this to your desired number
base_string <- "vector[nObs_%group%] fi_%group%; vector[nObs_%group%] logkobs_%group%; array[nAnalyte_subset_%group%] int<lower=1> starta_%group%; array[nAnalyte_subset_%group%] int<lower=1> enda_%group%;"
...
#mod12 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
mod12 <- cmdstan_model(here::here(mod_model_dir, model_name_ext),cpp_options = list(stan_threads = TRUE))
```

We used optimization for initial testing:
```{r stan-optimize, eval = FALSE}
fit_opt <- mod12$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod12-opt",
  init = init,
  iter=3000
)
...
```{r stan-sample, eval = FALSE}
 ...
 #fit$save_object(file = here(mod_output_dir,"mod12-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod12-*.csv')
...
```{r summary}
fit$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","sdpi","nu"), max_rows=13)
```
...
mod12gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod12gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
<!-- mod12cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1")) -->
<!-- ``` -->
...
<!-- ```{r summary-cv} -->
<!-- fitcv$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","rhogp","sdpi","nu"), max_rows=13) -->
<!-- ``` -->
...
<!-- mod12cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext)) -->
<!-- fitcvgq  <- mod12cvgq$generate_quantities(fitcv, -->
<!--                                           data = datastructcv, -->
<!--                                           seed = 123, -->
<!--                                           parallel_chains = 8, -->
<!--                                           output_dir = modgq_output_dir, -->
