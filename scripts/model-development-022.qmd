...
distance[similarity_matrix_corrected>=0.5]= 0.0
...
mod22 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:

```{r stan-optimize, eval = FALSE}
fit_opt <- mod22$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod22-opt",
  init = init,
  iter=3000
)

fit_opt$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","rhogp","sdpi"), max_rows=13)
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod22-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod22-*.csv')
...
```{r summary}
fit$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","rhogp","sdpi"), max_rows=13)
```
...
mod22gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod22gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
mod22cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
```{r summary-cv}
fitcv$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","rhogp","sdpi"), max_rows=13)
```
...
mod22cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod22cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
