---
title: "QSRR revisited. Model 009. GP attempt. OpenCL"
author:
  - name: "Paweł Wiczling*"
    affiliations:
...
Wsparcie OpenCL na węzłach:
- A100 80GB (kolejka: "gpu-a100-80gb"),
- H100 (kolejka: "gpu-h100").
Wersja platformy: "OpenCL 3.0 CUDA 12.3.68".
...
distance_ltri_rcdk_df <- read.csv(file = here(data_deliv_dir,"distance_ltri_rcdk.csv"), header = TRUE)
distance <- make_distance_matrix_fun(distance_ltri_rcdk_df)
...
datastruct <- with(data,
                  list(nAnalytes=length(unique(data$ID)),
                       ...
                       distance_x = 1-similarity_matrix,
                       start = start,
                       end= end,
                       logkobs=logk,
                       run_estimation=1))
...
init <- function(){
  list(   logkwHat  = rnorm(1,2,2),
          ...
          rhogp = 1 / rgamma(1, shape = 5, rate = 2),
          pilogkw = rep(0,nK),
          piS1 = rep(0,nK),
...
mod9 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))

# mod9 <-  cmdstan_model(here::here(mod_model_dir, model_name_ext),
# include_paths=here::here(mod_model_dir),                                      
# cpp_options=list(USER_HEADER= here::here(mod_model_dir, "kroneker.hpp")), 
...
fit_opt <- mod9$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod9-opt",
  init = init,
  iter=3000
)

fit_opt$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","rhogp","sdpi"), max_rows=12)
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod9-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod9-*.csv')
...
```{r summary}
fit$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","rhogp","sdpi"), max_rows=12)
```
...
mod9gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod9gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
top_100_indices <- order(upper_tri_values)[1:100]
...
mod9cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))                
```
...
```{r summary-cv}
fitcv$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","rhogp","sdpi"), max_rows=12)
```
...
mod9cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod9cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
