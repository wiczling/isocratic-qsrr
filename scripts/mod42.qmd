...
init_aprox %>%
  ggplot(aes(x=logP,y=logkw,color=factor(chargesB0+chargesA0)))+
  geom_point()+
  labs(color="charges")

init_aprox %>%
  ggplot(aes(x=logP,y=S1,color=factor(chargesB0+chargesA0)))+
  geom_point()+
  labs(color="charges")
```
...
init <- function(){
  list(   logkwHat  = rnorm(1,2,2),
          ...
          etadlogkw = rep(0,nA0+nB0+nGroupsA+nGroupsB),
          etadS1 = rep(0,nA0+nB0+nGroupsA+nGroupsB), 
...
bayesplot::mcmc_trace(fit$draws(c("omega","kappa","rho[1,2]","sigma"))) 
```
...
mcmc_intervals(fit$draws(c("omega","kappa","rho[1,2]","sigma")))
```
