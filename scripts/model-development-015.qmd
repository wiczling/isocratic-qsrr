...
mod15 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:

```{r stan-optimize, eval = FALSE}
fit_opt <- mod15$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod15-opt",
  init = init,
  iter=3000
)
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod15-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod15-*.csv')
...
mod15gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod15gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
<!-- mod15cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))                 -->
<!-- ``` -->
...
<!-- mod15cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext)) -->
<!-- fitcvgq  <- mod15cvgq$generate_quantities(fitcv, -->
<!--                                           data = datastructcv, -->
<!--                                           seed = 123, -->
<!--                                           parallel_chains = 8, -->
<!--                                           output_dir = modgq_output_dir, -->
