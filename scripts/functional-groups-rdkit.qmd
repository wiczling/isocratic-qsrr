...
id = nodes %>% filter(group==5) %>% pull(id)
similarity_ltri_rcdk_df%>%filter(row %in% id)
...
```{r partial correlations-matrixplot}
...
ids <- nodes %>%
  filter(group_combined == ugroup[i]) %>%
  arrange(group) %>%
  pull(id)
   
#ids <- nodes$id[nodes$group_combined == ugroup[i]] orderd by id

p1<-ggcorrplot(simplified_partial_correlations[ids,ids], 
                outline.color = NA,
                tl.cex = 8,
                show.legend = FALSE,
                title = paste("correlated:", "group = ", ugroup[i]))
print(p1)
}

ggcorrplot(simplified_partial_correlations[idx_uncorelatted,
                               idx_uncorelatted], 
                outline.color = NA,
                tl.cex = 8,
                show.legend = FALSE,
...
map(1:16, \(x) plotting_i(x, similarity_subset)) %>% wrap_plots(ncol = 4)
