---
title: "QSRR revisited. Model 013. GSEM"
author:
  - name: "Paweł Wiczling*"
    affiliations:
...
Structural Equation Model with fairly simplified correlation structure.
...
# Approximate the matrix to the nearest positive definite one
similarity_matrix_corrected <- as.matrix(Matrix::nearPD(similarity_matrix, corr = TRUE)$mat)
results <- marginal_to_partial_correlations(marginal_corr = similarity_matrix_corrected,rho = 0.1)

correlation_treshold = 0.25
input_indexes <- similarity_to_ltr_fun(results$partial_corr) %>% filter(similarity>correlation_treshold, row!=col)
uid = unique(c( input_indexes$row, input_indexes$col))
nodes <- data.frame(id = uid,label = paste(uid))
edges <- data.frame(from =  input_indexes$row, to = input_indexes$col)
...
tabulate(nodes$group)

df <- nodes
repeat {
  group_sizes <- df %>% dplyr::count(group, name = "size")
  if (all(group_sizes$size >= 40)) break
...
ugroup = unique(nodes$group_combined)
nGroups = length(ugroup)

simplified_partial_correlations = results$partial_corr
simplified_partial_correlations[simplified_partial_correlations<=correlation_treshold]=0.0;

group = nodes$group_combined;
partial_correlation_subset <- simplified_partial_correlations[idx_corelatted,idx_corelatted]
L_K_prec <- t(chol(partial_correlation_subset))

# Initialize lists
nD_groups <- list()
point_mu_lower_groups <- list()
point_sd_lower_groups <- list()
L_K_prec_group <- list()
nAnalyte_subset_groups <- list()
idx_corr_group <- list()
ugroup = unique(group)
nGroups = length(ugroup)
...
# Loop
for (g in 1:nGroups) {
  idx = nodes$group_combined == ugroup[g]
  group_data <- partial_correlation_subset[idx,idx]
  nAnalyte_subset_groups[[g]]=nrow(group_data)
  nD_groups[[g]] <-nAnalyte_subset_groups[[g]] * (nAnalyte_subset_groups[[g]] - 1) / 2
  point_mu_lower_groups[[g]] <- group_data[lower.tri(group_data, diag = FALSE)]
  point_mu_lower_groups[[g]] <- ifelse(point_mu_lower_groups[[g]] == 0, 0, 0.5)
  point_sd_lower_groups[[g]] <- ifelse(point_mu_lower_groups[[g]] == 0, 0.01, 0.5)
  L_K_prec_group[[g]] <- t(chol(group_data))
  idx_corr_group[[g]]<-which(idx)
}

# Create the datastruct list
datastruct <- list(
    nAnalytes = length(unique(data$ID)),
...
# Add group-specific variables
  for (g in 1:nGroups) {
    datastruct[[paste0("nD_", g)]] <- nD_groups[[g]]
    datastruct[[paste0("nAnalyte_subset_", g)]] <- nAnalyte_subset_groups[[g]]
    datastruct[[paste0("point_mu_lower_", g)]] <- point_mu_lower_groups[[g]]
    datastruct[[paste0("point_sd_lower_", g)]] <- point_sd_lower_groups[[g]]
    datastruct[[paste0("point_sd_lower_", g)]] <- point_sd_lower_groups[[g]]
    datastruct[[paste0("idx_corr_", g)]] <- idx_corr_group[[g]]
  }

# initialize the values for each variable in each chain:
init <- function() {
  init_list <- list(
    logkwHat = rnorm(1, 2, 2),
    S1Hat = rnorm(1, 5, 1),
  ...
  # Add group-specific L_K_prec_<group> fields
  for (g in 1:nGroups) {
    init_list[[paste0("L_K_prec_", g)]] <- L_K_prec_group[[g]]
    init_list[[paste0("param_corr_", g)]] <-cbind(init_aprox$logkw[idx_corelatted[idx_corr_group[[g]]]],               init_aprox$S1[idx_corelatted[idx_corr_group[[g]]]])
...
n <- nGroups  # Change this to your desired number
base_string <- "sparam_pop[idx_corr[idx_corr_%group%],1:2]=param_corr_pop_%group%;"
result <- sapply(1:n, function(i) {gsub("%group%", i, base_string)})
result <- gsub(";", ";\n", result)
cat(result, sep = "\n")
```
...
### Plot
```{r test-accuracy}
partial_to_marginal <- function(partial_cor) {
  covariance <- solve(partial_cor)
  marginal_cor <- cov2cor(covariance)
  return(marginal_cor)
}
marginal_corr <- similarity_to_ltr_fun(partial_to_marginal(results$precision_matrix)) %>% filter(row!=col)
similarity <- similarity_to_ltr_fun(similarity_matrix_corrected) %>% filter(row!=col)
plot(similarity$similarity, marginal_corr$similarity)
```
...
mod14 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:
```{r stan-optimize, eval = FALSE}
fit_opt <- mod14$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod14-opt",
  init = init,
  iter=3000
)

fit_opt$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","sdpi"), max_rows=13)
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod14-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod14-*.csv')
...
```{r summary}
fit$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","sdpi"), max_rows=13)
```
...
mod14gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod14gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
```{r correlation-group1}
L_K_prec_1 <- L_K_prec_group[[1]]
rho_1_df <- similarity_to_ltr_fun(L_K_prec_1 %*% t(L_K_prec_1)) %>% filter(similarity>0, row!=col) %>% rename(correlation = similarity)

rho_1_df_posterior <-  draws_gen_df %>%
  tidybayes::spread_draws(rho1[row,col]) %>%
  group_by(row,col)%>%
  summarise(correlation_posterior = mean(rho1))%>% 
  filter(correlation_posterior>0.01, row!=col, row>col)

rho_1_df_posterior<-rho_1_df_posterior%>%
  left_join(rho_1_df)

rho1_vars <- paste0("rho1[", rho_1_df_posterior$row, ",", rho_1_df_posterior$col, "]")
draws_array <- posterior::as_draws_array(draws_gen_df %>% select(all_of(rho1_vars)))
p1 <- bayesplot::mcmc_intervals(draws_array)

rho2_vars <- paste0("rho1[", rho_1_df_posterior$row-1, ",", rho_1_df_posterior$col-1, "]")
rho2_vars <- str_replace(rho2_vars, "rho1\\[27,25\\]", "rho1\\[27,23\\]")
rho2_vars <- str_replace(rho2_vars, "rho1\\[47,45\\]", "rho1\\[47,44\\]")
rho2_vars <- str_replace(rho2_vars, "rho1\\[51,35\\]", "rho1\\[51,34\\]")
rho2_vars <- str_replace_all(rho2_vars, ",0]", ",1]")

draws_array <- posterior::as_draws_array(draws_gen_df %>% select(all_of(rho2_vars)))
p2 <- bayesplot::mcmc_intervals(draws_array)

p1+p2

selected = data.frame(row = idx_corelatted[rho_1_df_posterior$row], 
                      col = idx_corelatted[rho_1_df_posterior$col])
selected
```

```{r prior-posterior-corr}
partial_correlation_summary = function(group, L_K_prec_group=L_K_prec_group, draws_gen_df=draws_gen_df){
 
  L_K_prec <- L_K_prec_group[[group]]
  df_sim <- similarity_to_ltr_fun(L_K_prec %*% t(L_K_prec)) %>%
    filter(similarity > 0, row != col) %>%
    rename(correlation = similarity)

  param_name <- paste0("rho", group)
  param_sym <- parse_expr(glue::glue("{param_name}[row,col]"))

  results <- draws_gen_df %>%
    tidybayes::spread_draws(!!param_sym) %>%
    group_by(row, col)%>%
    summarise(correlation_posterior = mean(!!sym(param_name)), .groups = "drop") %>%
    filter(correlation_posterior > 0.01, row != col, row > col) %>%
    left_join(df_sim, by = c("row", "col"))

  return(results)
}

partial_correlation_results <- map_df(1:8, function(x) {
  partial_correlation_summary(
  group = x,
  L_K_prec_group = L_K_prec_group,
  draws_gen_df = draws_gen_df
  )})

partial_correlation_results %>%
  ggplot(aes(x=correlation,y=correlation_posterior))+
  geom_point()+
  labs(x = "Partial correlation (assummed)", y ="Partial correlations (a posteriori)")
```
...
mod14cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
```{r summary-cv}
fitcv$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","sdpi","nu"), max_rows=13)
```
...
mod14cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod14cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
...
draws_gen_df_cv  <- as_draws_df(x$generated_quantities)
```
