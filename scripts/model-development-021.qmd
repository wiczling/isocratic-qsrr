...
distance_x = 1-similarity_matrix_corrected
diag(distance_x)<-1
mines = apply(distance_x,2,min)
idx_uncorelatted = which(mines>0.5)
idx_corelatted = which(mines<=0.5)
...
mod21 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
We used optimization for initial testing:

```{r stan-optimize, eval = FALSE}
fit_opt <- mod21$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod21-opt",
  init = init,
  iter=3000
)

fit_opt$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","alpha","sdpi","nu"), max_rows=13)
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod21-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod21-*.csv')
...
mod21gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod21gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
# https://pmc.ncbi.nlm.nih.gov/articles/PMC5321813/ use decorelations for assessment
etas <- draws_gen_df %>%
  select(starts_with("seta_ind"))
...
mod21cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod21cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod21cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
