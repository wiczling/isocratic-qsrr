...
# Matri normal distribution - test
...
# Create correlation matrices
rho1 <- matrix(rho_row_true, nrow = n, ncol = n)
diag(rho1) <- 1
L_rho1 <- t(chol(rho1))

rho2 <- matrix(c(1, rho_col_true, rho_col_true, 1), nrow = 2)
L_rho2 <- t(chol(rho2))

pred_two_analytes <- function(n) {
  eta <- matrix(rnorm(2 * n, 0, 1), nrow = n)
  f <- L_rho1 %*% eta %*% t(diag(c(omega1, omega2)) %*% L_rho2)
  return(c(f[1, 1], f[1, 2], f[2, 1]))
}
...
# Print results
cat(sprintf("Simulated sd_logkw1 = %.2f: true = %.2f\n", stats$sd_logkw1,omega1),
    sprintf("Simulated sd_S1 = %.2f: true = %.2f\n", stats$sd_S1,omega2),
    sprintf("Simulated sd_logkw2 = %.2f: true = %.2f\n", stats$sd_logkw2,omega1),
    sprintf("Simulated rho_logkw1_S1 = %.2f: true =%.2f\n", stats$rho_logkw1_S1,rho_col_true),
    sprintf("Simulated rho_logkw1_logkw2 = %.2f: true = %.2f\n", stats$rho_logkw1_logkw2,rho_row_true))
