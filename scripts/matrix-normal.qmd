...
Simulations from the Matrix Normal Distribution
$$
\begin{bmatrix}
\text{log}k_{w1} & S_1 \\
\text{log}k_{w2} & S_2
...
pred_two_analytes <- function(n) {
  ...
  return(c(f[1, 1], f[1, 2], f[2, 1], f[2, 2]))
}
...
sim_data <- replicate(num_sim, pred_two_analytes(n)) %>%
  t() %>%
  as.data.frame() %>%
  setNames(c("logkw1", "S1", "logkw2", "S2"))
...
plot1 <- ggplot(sim_data, aes(y = logkw1, x = logkw2)) +
  geom_point(alpha = 0.5) +
  theme_minimal() +
  labs( y = "logkw1", x = "logkw2")

plot2 <- ggplot(sim_data, aes(y = logkw1, x = S1)) +
  geom_point(alpha = 0.5) +
  theme_minimal() +
  labs( y = "logkw1", x = "S1")

plot3 <- ggplot(sim_data, aes(y = logkw2, x = S2)) +
  geom_point(alpha = 0.5) +
  theme_minimal() +
  labs( y = "logkw2", x = "S2")

plot4 <- ggplot(sim_data, aes(y = S1, x = S2)) +
  geom_point(alpha = 0.5) +
  theme_minimal() +
  labs( y = "S1", x = "S2")

# Display plots
figure_S2<-((plot1+plot2)/(plot3+plot4))
figure_S2
```

<!-- # Matrix-t distriution  -->
```{r matrix-t-distribution, eval=FALSE}
library(cmdstanr)
library(ggplot2)
library(gridExtra)
library(patchwork)
...
ggsave(here::here("deliv","figures","manuscript","supplement ", "matrix-normal.png"),
       plot=figure_S2, width = 20, height = 20, units = "cm", dpi = 300)
ggsave(here::here("deliv","figures","manuscript","supplement", "matrix-normal.pdf"),
       plot=figure_S2, width = 20, height = 20, units = "cm", dpi = 300)
```
