...
L_rho2 <- t(chol(rho2))
eta <- matrix(rnorm(2*2,0,1),nrow=2)
...
K<-simMat;
  ...
  #     K[i, j] = exp(-0.5/16 * (tsimMat[i,j])^2);
  #     K[j, i] = K[i, j];
  #   }
  # }
  # K[nAnalytes, nAnalytes] = 1 + delta;
  # 
  # K = diag_matrix(rep(1,1026)) * rho * t(diag_matrix(rep(1,1026))
  # Q <- chol(K)
...
rho[rho==1]<-0.999
diag(rho) <- 1
K = diag(1,1026) %*% as.matrix(rho) %*% t(diag(1,1026))
Q <- t(chol(rho))
