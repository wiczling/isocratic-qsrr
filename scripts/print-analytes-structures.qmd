...
Print 2D structures of analytes
...
```{r load-conda-environment-old, eval = FALSE}
...
save_png_fun <- function(i) {
  ...
  # Return a data frame with analyte name and HTML image tag
  data.frame(
...
# Apply the function and collect results into a data frame
results <- lapply(seq_along(smiles$smiles), save_png_fun)
results_df <- bind_rows(results)

# Create and print a kable table with images
kableExtra::kbl(results_df, 
                caption = "Analyte Names and Corresponding Structure Images",
                col.names = c("Analyte Name", "Structure"),
                escape = FALSE) %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                            full_width = FALSE) %>%
  print()

# Optionally save the table as HTML for inclusion in reports
