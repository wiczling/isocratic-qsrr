---
title: "QSRR revisited. Model 002"
author:
  - name: "Paweł Wiczling*"
    affiliations:
...
similarity_df <- read.csv(file = here(data_deliv_dir,"similarity_ltri_rcdk.csv"), header = TRUE)
...
mod2 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod2-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod2-*.csv')
...
mod2gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod2gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
mcmc_intervals(fit$draws(c("omega","rho[1,2]","sigma", "sdpi")))
```
...
param1_node = param1[equations_fg_ordered$node[equations_fg_ordered$parent>0]]
param2_node = param2[equations_fg_ordered$node[equations_fg_ordered$parent>0]] 
  
param1_parent = param1[equations_fg_ordered$parent[equations_fg_ordered$parent>0]]
param2_parent = param2[equations_fg_ordered$parent[equations_fg_ordered$parent>0]]

param1 <- param1_node-param1_parent
param2 <- param2_node-param2_parent

crit <- equations_fg_ordered$crit[equations_fg_ordered$parent>0]
fg <- equations_fg_ordered[equations_fg_ordered$parent>0,]

logPobs = datastruct$logPobs[equations_fg_ordered$node[equations_fg_ordered$parent>0]]
data_to_plot_param <- cbind(param1,param2,param1-param2,logPobs, crit)

data_to_plot_param <- as.data.frame(data_to_plot_param) 

colnames(data_to_plot_param) <- c(expression('etalogkwN'[i]),expression('etaS1mN'[i]),expression('etalogka'[i]),expression('logP'[i]),expression('crit'[i]))

p<-ggpairs(data_to_plot_param, columnLabels = colnames(data_to_plot_param) ,
        labeller = "label_parsed",upper = list(continuous = "points"))
...
similarity_df$etalogkw1 = param1[similarity_df$row]
similarity_df$etalogkw2 = param1[similarity_df$col]

similarity_df<-similarity_df %>%
  filter(row!=col) %>%
  mutate(r=etalogkw1-etalogkw2)

p1 =  similarity_df %>%
  ggplot(aes(x=similarity, y = r)) +
  geom_point(size=0.5, alpha=0.3)

binned_data <- similarity_df %>%
  mutate(bin = cut(similarity, breaks = seq(0, 1, by = 0.1), include.lowest = TRUE)) %>%
  group_by(bin) %>%
  summarise(sd_r = sd(r, na.rm = TRUE)) %>%
  mutate(mid_bin = seq(0, 1-0.1, by = 0.1) + 0.1/2)

p2 <- binned_data %>%
  ggplot(aes(x = mid_bin, y = sd_r)) +
  geom_point(alpha = 0.5, size = 2) +
  labs(x = "Midpoint of Similarity Bin", 
       y = "SD of r")

p1/p2

p3<-similarity_df %>%
  filter(similarity>0.0 & similarity<0.1) %>%
  ggplot(aes(x=r))+
  geom_histogram()+
  xlim(-10,10)

p4<-similarity_df %>%
  filter(similarity>0.9 & similarity<1) %>%
  ggplot(aes(x=r))+
  geom_histogram()+
  xlim(-10,10)
