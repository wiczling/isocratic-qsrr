...
Modeling HPLC data taking the similarity of analytes into account.
...
# Create a group vector for all matrix indices 1:n
group_vec <- group_membership[as.character(1:n)]  # Ensure names match

# Build a group-equality mask (same group = TRUE, else FALSE)
...
mod27 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:

```{r stan-optimize, eval = FALSE}
fit_opt <- mod27$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod27-opt",
  init = init,
  iter=3000
)
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod27-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod27-*.csv')
...
mod27gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod27gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
mod27cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod27cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod27cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
