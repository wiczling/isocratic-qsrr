...
row_idx = input_indexes$row
col_idx = input_indexes$col

uid = unique(c(row_idx, col_idx))
...
init <- function() {
  init_list <- list(
    ...
    L_K_prec = L_K_prec,  # Original L_K_prec (list or single object)
    nu = 20,
    pilogkw = rep(0, nK),
    piS1 = rep(0, nK),
...
base_string <- "int<lower=1> nD_%group%; int<lower=1> nAnalyte_subset_%group%; vector[nD_%group%] point_mu_lower_%group%; vector[nD_%group%] point_sd_lower_%group%; array[nAnalyte_subset_%group%] int idx_corr_%group%;"
...
mod11 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:
```{r stan-optimize, eval = FALSE}
fit_opt <- mod11$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod11-opt",
  init = init,
  iter=3000
)
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod11-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod11-*.csv')
...
```{r summary}
fit$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","rhogp","sdpi","nu"), max_rows=13)
```
...
mod11gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod11gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
mod11cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod11cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod11cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
