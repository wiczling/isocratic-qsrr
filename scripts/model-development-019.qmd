...
L_K <- t(chol(similarity_matrix_corrected))
...
input_indexes <- similarity_to_ltr_fun(results$partial_corr) %>% filter(similarity>0.1, row!=col)
uid = unique(c(input_indexes$row, input_indexes$col))
nodes <- data.frame(id = uid,label = paste(uid))
edges <- data.frame(from = input_indexes$row,
                    to = input_indexes$col)
...
df$group_final <- as.integer(factor(df$group))
group_vec_final <- df$group_final
table(group_vec_final)
length(unique(group_vec_final))  # New number of groups

group = group_vec_final;
partial_correlation_subset <- results$partial_corr[idx_corelatted,idx_corelatted]
L_K_prec <- t(chol(partial_correlation_subset))

# Initialize lists to store nD, point_mu_lower, and point_sd_lower for each group
nD_groups <- list()
...
# Loop
for (g in 1:nGroups) {
  group_data <- partial_correlation_subset[group==ugroup[g],group==ugroup[g]]
  ...
  idx_corr_group[[g]]<-which(group==ugroup[g])
}
...
# Add group-specific nD, point_mu_lower, and point_sd_lower with _group suffix
  for (g in 1:nGroups) {
...
mod19 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
We used optimization for initial testing:
```{r stan-optimize, eval = FALSE}
fit_opt <- mod19$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod19-opt",
  init = init,
  iter=3000
)
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod19-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod19-*.csv')
...
mod19gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod19gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
### Prior posterior correlations

```{r correlation-group1}
L_K_prec_2 <- L_K_prec_group[[2]]
rho_2_df <- similarity_to_ltr_fun(L_K_prec_2 %*% t(L_K_prec_2)) %>% filter(similarity>0, row!=col) %>% rename(correlation = similarity)

rho_2_df_posterior <-  draws_gen_df %>%
  tidybayes::spread_draws(rho2[row,col]) %>%
  group_by(row,col)%>%
  summarise(correlation_posterior = mean(rho2))%>% 
  filter(correlation_posterior>0.01, row!=col, row>col)

rho_2_df_posterior<-rho_2_df_posterior%>%
  left_join(rho_2_df) 

rho2_vars <- paste0("rho2[", rho_2_df_posterior$row, ",", rho_2_df_posterior$col, "]")
...
foo=similarity_to_ltr_fun(cor_matrix_decorrelated)%>%
  rename(corr = similarity) %>%
  left_join(similarity_to_ltr_fun(similarity_matrix_corrected)) %>%
  filter(row!=col) %>%
  left_join(nodes%>%rename(row=id)) %>%
  ...
  filter(group_combined == 0)%>%
  filter(similarity>0.95)
```

### Posterior correlations v2
```{r posterior-corelations-v2}

df <- draws_gen_df %>% select(starts_with("rho2")) %>% summarise_all(mean, na.rm = TRUE)

df_long <- df %>%
  pivot_longer(cols = everything(), names_to = "rho_index", values_to = "rho") %>%
  mutate(
    i = str_extract(rho_index, "(?<=\\[)\\d+"),
    j = str_extract(rho_index, "(?<=,)\\d+(?=\\])"),
    i = as.integer(i),
    j = as.integer(j)
  )%>%
  filter(i > j) %>%
  mutate(rho_similarity = datastruct$point_mu_lower_2)

p1 = df_long %>% ggplot(aes(x=rho_similarity, y= rho))+
  geom_point()
...
mod19cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
```{r summary-cv}
fitcv$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","sdpi"), max_rows=13)
```
...
mod19cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod19cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
...
p1_1<-fun_plot_ind_pop(idx_analytes_out[1:20])
p1_2<-fun_plot_ind_pop(idx_analytes_out[21:40])
p1_3<-fun_plot_ind_pop(idx_analytes_out[41:57])
print(p1_1)
print(p1_2)
print(p1_3)
