...
datastruct <- with(data,
                  list(nAnalytes=length(unique(data$ID)),
                       ...
                       similarity_s = similarity_matrix_corrected,
                       start = start,
                       end= end,
                       logkobs=logk,
                       run_estimation=1))
...
mod31 <- cmdstan_model(here::here(mod_model_dir, "model_name_ext"), stanc_options = list("O1"))
```

We used optimization for initial testing:

```{r stan-optimize, eval = FALSE}
fit_opt <- mod31$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod31-opt",
  init = init,
  iter=3000
)
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod31-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod31-1.csv mod31-2.csv mod31-3.csv mod31-4.csv mod31-5.csv mod31-6.csv mod31-7.csv mod31-8.csv')
...
mod31gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod31gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
datax <-  jsonlite::fromJSON(here("model/stan/mod31cv/standata.json"))
...
mod31cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod31cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod31cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
