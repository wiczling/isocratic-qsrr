...
K = 1*(simMat_adj[idx_corelatted,idx_corelatted]-0.6)/0.6
diag(K)<-1.00001
L_K=t(chol(K))
...
# Most similar to 111
ggplot(filter(data, ID %in% which(K[111,]>alpha^2*0.95)), aes(x=fi, y=logk-(0.76*(logP_ACD-2.5)-0.56*(logP_ACD-2.5)*fi), group=ID)) + 
  geom_line(aes(color=logP_ACD)) + 
  labs(x = "\u03C6", y = expression(log~k[obs])) + 
  theme_gray(base_size = 14) + theme(legend.position="none") 
```
...
bayesplot::mcmc_trace(fit$draws(c("omega","rhoc[2,1]","sigma"))) 
```
...
mcmc_intervals(fit$draws(c("omega","rhoc[2,1]","sigma")))
```
