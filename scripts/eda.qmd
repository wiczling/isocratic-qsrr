...
p1<-ggplot(data=data_ACD)+
  geom_histogram(aes(x=MW))+
  xlab("molecular mass")+
  ylab("count")

p2<-ggplot(data=data_ACD)+
...
init_aprox$MW <-data_ACD$MW
init_aprox$logPobs <- data_ACD$logP
```
...
init_aprox %>%
  ggplot(aes(x=logkw,y=S1,color=factor(fngr)))+
  geom_point()+
  labs(color="charges")
```
