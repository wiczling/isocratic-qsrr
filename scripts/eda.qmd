...
1.  Number of analytes: `r length(unique(data$ID))`
2.  Number of observations: `r length(data$ID)`
3.  Molecular mass: `r round(mean(descriptor_df$MolWt),2)` $\pm$ `r round(sd(descriptor_df$MolWt),2)`
3.  logP: `r round(mean(descriptor_df$LogP),2)` $\pm$ `r round(sd(descriptor_df$LogP),2)`
4.  Number of analytes with 0, 1 and 2 dissociation steps: `r tabulate(as.factor(dissociation$R))`
5.  Number of acidic and basic groups: `r round(length(dissociation$pKaslitA),2)` and `r round(length(dissociation$pKaslitB),2)`
