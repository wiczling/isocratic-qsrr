...
ggcorrplot(simMat[100:200,100:200],tl.cex=4)

distance_square <- -2*log(simMat)
diag(distance_square)<-0
alpha <- 1
rho_gp <- 0.75 ; # rho_gp <- 0.7 used
K = alpha^2 * exp(-1/2/rho_gp^2*distance_square)
K = as.matrix(K)
diag(K)<-1.00001
L_K=t(chol(K))
L_K[1:5,1:5]
ggcorrplot(K[100:200,100:200],tl.cex=4)
...
ggplot(filter(data, ID %in% which(K[111,]>0.95)), aes(x=fi, y=logk-(0.76*(logP_ACD-2.5)-0.56*(logP_ACD-2.5)*fi), group=ID)) + 
  geom_line(aes(color=logP_ACD)) + 
  labs(x = "\u03C6", y = expression(log~k[obs])) + 
  theme_gray(base_size = 14) + theme(legend.position="none") 
...
analytes_idx = (100:115) # c(1:20); #c(1:9, 501,510, 626) (100:120)
