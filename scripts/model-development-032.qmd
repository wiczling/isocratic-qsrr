...
init <- function() {
  init_list <- list(
    ...
    alpha = min(max(0,rnorm(1, 0.5, 0.25)),1),
...
mod32 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
We used optimization for initial testing:
```{r stan-optimize, eval = FALSE}
fit_opt <- mod32$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod32-opt",
  init = init,
  iter=3000
)

fit_opt$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","alpha", "sdpi"), max_rows=14)
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod32-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod32-*.csv')
...
mod32gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod32gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
df <- draws_gen_df %>% select(starts_with("rho6[")) %>% summarise_all(mean, na.rm = TRUE)
...
df_long <- df %>%
  ...
  filter(i > j) %>%
  mutate(rho_similarity = datastruct$point_mu_lower_6)
...
mod32cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod32cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod32cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
