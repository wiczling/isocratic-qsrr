...
similarity_ltri_rcdk_subset<-similarity_to_ltr_fun(similarity_matrix_corrected) %>% filter(similarity>=1/3, row!=col)
...
repeat {
  ...
  if (all(group_sizes$size >= 50)) break
...
mod30 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:

```{r stan-optimize, eval = FALSE}
fit_opt <- mod30$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod30-opt",
  init = init,
  iter=3000
)
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod30-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod30-*.csv')
...
mod30gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod30gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
mod30cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod30cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod30cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
...
```{r ind-pop-predictions-cv}
 ...
 # datax <- fromJSON(here("model/stan/mod30cv/standata.json"))
