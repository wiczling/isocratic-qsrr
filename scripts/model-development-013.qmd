...
for (g in 1:nGroups) {
  ...
  point_sd_lower_groups[[g]] <- ifelse(point_mu_lower_groups[[g]] == 0, 0.01, 0.2)
...
mod13 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:
```{r stan-optimize, eval = FALSE}
fit_opt <- mod13$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod13-opt",
  init = init,
  iter=3000
)

fit_opt$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","sdpi","nu"), max_rows=13)
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod13-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod13-*.csv')
...
mod13gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod13gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
```{r show-correlation-for-selected}
...
mod13cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod13cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod13cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
