...
```{r}
extract_attachment_info_df <- function(smiles1, smiles2, .mcs_params=mcs_params) {
  ...
  for (atom_idx in match_atoms) {
    ...
    for (bond in atom$GetBonds()) {
      ...
      if (!(nbr_idx %in% match_atoms)) {
        ...
        bond_stereo <- as.character(bond$GetStereo())
        
        attachment_info[[length(attachment_info) + 1]] <- list(
          frag_atom_idx = atom_idx,
          frag_atom_symbol = atom$GetSymbol(),
          frag_formal_charge = atom$GetFormalCharge(),
          frag_degree = atom$GetDegree(),
          frag_total_Hs = atom$GetTotalNumHs(),
          frag_hybridization = as.character(atom$GetHybridization()),
          
          ext_atom_idx = nbr_idx,
          ext_atom_symbol = nbr_atom$GetSymbol(),
          ext_formal_charge = nbr_atom$GetFormalCharge(),
          ext_degree = nbr_atom$GetDegree(),
          ext_total_Hs = nbr_atom$GetTotalNumHs(),
          ext_hybridization = as.character(nbr_atom$GetHybridization()),
          
          bond_type = bond_type,
          bond_aromatic = bond_is_aromatic,
          bond_conjugated = bond_is_conjugated,
  ...
  if (length(attachment_info) == 0) {
    return(data.frame())
  } else {
    return(do.call(rbind, lapply(attachment_info, as.data.frame)))
  }
}
...
foo <- extract_attachment_info_df(smiles1, smiles2)
```
