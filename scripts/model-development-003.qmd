---
title: "QSRR revisited. Model 003"
author:
  - name: "Paweł Wiczling*"
    affiliations:
...
Simple model with functional group effects.
...
equations_fg_ordered= read.csv(file = here(data_deliv_dir,"equations_fg_ordered.csv"), header = TRUE)
equations_fg_ordered<-equations_fg_ordered%>% select(node, parent, crit)
...
```{r rearange all the required variables}

#rearange the data so it matches the order
id_reordered = equations_fg_ordered$node; # new order
data_reordered <- data %>% arrange(factor(ID, levels = id_reordered))
data_reordered$Idi = match(data_reordered$ID, unique(data_reordered$ID))
logPobs_reordered <- data_ACD$logP[id_reordered]
idx_orphan = which(equations_fg_ordered$parent==0)
functional_groups_reordered = functional_groups[id_reordered,]
#functional_groups_reordered[idx_orphan,] = functional_groups[idx_orphan,]
```

```{r stan-setup}
# create Stan data set: data
nObs <- length(data_reordered$ID)
nAnalytes <- length(unique(data_reordered$ID))
nOrphan <- length(idx_orphan)
start <- (1:nObs)[!duplicated(data_reordered$ID)]
end <- c(start[-1] - 1, nObs)
nK = ncol(functional_groups_reordered)
datastruct <- with(data_reordered,
                  list(nAnalytes=length(unique(data_reordered$ID)),
                       nObs=length(data_reordered$ID),
                       nOrphan = nOrphan,
                       analyte=match(data_reordered$ID, unique(data_reordered$ID)),
                       parents = match(equations_fg_ordered$parent, equations_fg_ordered$node),
                       fi=data_reordered$fi,
                       logPobs=logPobs_reordered,
                       nK = nK,
                       fgrp = functional_groups_reordered,
                       start = start,
                       end= end,
                       logkobs=logk,
                       run_estimation=1))
datastruct$parents[is.na(datastruct$parents)] = 0
...
init <- function(){
  list(   logkwHat  = rnorm(1,2,2),
          ...
          omega1 = c(1,1)*exp(rnorm(2, 0, 0.5)),
          omega2 = c(1,1)*exp(rnorm(2, 0, 0.5)),
          rho1 = matrix(c(1,0.75,0.75,1), nrow=2),
          rho2 = matrix(c(1,0.75,0.75,1), nrow=2),
          #param = cbind(matrix(2, 1, nrow=nSteps), matrix(5, 1, nrow=nSteps)),
          param = cbind(init_aprox$logkw[id_reordered],init_aprox$S1[id_reordered]),
          pilogkw = rep(0,nK),
          piS1 = rep(0,nK),
...
mod3 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:

```{r stan-optimize, eval = FALSE}
fit_opt <- mod3$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod3-opt",
  init = init,
  iter=3000
)
...
```{r plots-optimize}
analytes_names_reorder = analytes_names[id_reordered,]%>%mutate(ID=1:n())

fit_opt <- cmdstanr::as_cmdstan_fit(c(
                                  paste0(mod_output_dir,"/",  model_name, "-opt-1.csv")
                                ))

data_reordered$logkopt<-fit_opt$mle("logkx")

set.seed(123)
p1<-data_reordered %>%
  filter(ID %in% sample(unique(data_reordered$ID),12)) %>%
  left_join(analytes_names_reorder) %>%
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod3-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod3-*.csv')
...
mod3gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod3gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
bayesplot::mcmc_trace(fit$draws(c("omega1","rho1[1,2]","omega2","rho2[1,2]","sigma","sdpi"))) 
...
mcmc_intervals(fit$draws(c("omega1","rho1[1,2]","omega2","rho2[1,2]","sigma","sdpi")))
```
...
p <-draws_gen_df %>%
  ...
  geom_point(data = filter(data_reordered,Idi %in% analytes_idx),aes(x=fi, y=logk)) + 
  facet_wrap(~Analyte)+
...
data_to_plot_param<-as.data.frame(data_to_plot_param)
data_to_plot_param$crit = equations_fg_ordered$crit
data_to_plot_param$crit[1:nOrphan] = 8
data_to_plot_param$group <- factor(c(0 * c(1:nOrphan), 1 + 0 * c((1 + nOrphan):nAnalytes)), 
                                   labels = c("Group 0", "Group 1"))  # Convert to factor with meaningful 

p <- ggpairs(
  data_to_plot_param,
  columns = 1:(ncol(data_to_plot_param) - 1),  
  mapping = aes(color = group, alpha = 0.6),  
  columnLabels = colnames(data_to_plot_param)[1:(ncol(data_to_plot_param) - 1)],
  labeller = "label_parsed",
  upper = list(continuous = wrap("points", size = 2, shape = 16)),  
  lower = list(continuous = wrap("points", size = 2, shape = 16)),  
