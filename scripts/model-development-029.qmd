...
Modeling HPLC data taking the similarity of analytes into account. Log P and functional groups calculated using rdkit.
...
& R_{i} \sim \text{MN}(\theta_R  + \beta \cdot log P_i +\pi_R \cdot X_i , K, \Omega) \\
...
mod29 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:

```{r stan-optimize, eval = FALSE}
fit_opt <- mod29$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod29-opt",
  init = init,
  iter=3000
)
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod29-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod29-1.csv mod29-2.csv mod29-3.csv mod29-4.csv mod29-5.csv mod29-6.csv mod29-7.csv mod29-8.csv')
...
```{=html}
<pre>
 [1] "Checking sampler transitions treedepth."                                                                  [2] "Treedepth satisfactory for all transitions."                                                              [3] ""                                                                                                         [4] "Checking sampler transitions for divergences."                                                            [5] "No divergent transitions found."                                                                          [6] ""                                                                                                         [7] "Checking E-BFMI - sampler transitions HMC potential energy."                                              [8] "E-BFMI satisfactory."                                                                                     [9] ""                                                                                                        [10] "Rank-normalized split effective sample size satisfactory for all parameters."                            [11] ""                                                                                                        [12] "The following parameters had rank-normalized split R-hat greater than 1.01:"                             [13] "  sdpi[4], param_corr[252,1], param_corr[328,1], param_corr[405,1], param_corr[328,2], param_corr[405,2], param_uncorr[14,1], param_uncorr[14,2], logkx[621], logkx[622], logkx[2346], logkx[2347], logkx[2862], logkx[2863], logkx[2864], logkx[3365], logkx[3366], param[95,1], param[394,1], param[510,1], param[626,1], param[95,2], param[510,2], param[626,2], lprior"
[14] "Such high values indicate incomplete mixing and biased estimation."                                      [15] "You should consider regularizating your model with additional prior information or a more effective parameterization."                                                                                            [16] ""                                                                                                        [17] "Processing complete."
</pre>
```
...
mod29gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod29gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
mod29cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod29cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod29cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
...
fun_plot_ind_pop(idx_analytes_out[1:10])
fun_plot_ind_pop(idx_analytes_out[11:20])
fun_plot_ind_pop(idx_analytes_out[21:30])
fun_plot_ind_pop(idx_analytes_out[31:40])
fun_plot_ind_pop(idx_analytes_out[41:50])
fun_plot_ind_pop(idx_analytes_in[1:10])
fun_plot_ind_pop(idx_analytes_in[11:20])
fun_plot_ind_pop(idx_analytes_in[21:30])
fun_plot_ind_pop(idx_analytes_in[31:40])
fun_plot_ind_pop(idx_analytes_in[41:50])
