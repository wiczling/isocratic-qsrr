...
mod24 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
We used optimization for initial testing:

```{r stan-optimize, eval = FALSE}
fit_opt <- mod24$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod24-opt",
  init = init,
  iter=3000
)

fit_opt$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","alpha","sdpi","tau", "nu"), max_rows=13)
```
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod24-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod24-*.csv')
...
```{r summary}
fit$print(variables = c("logkwHat","S1Hat","beta","S2Hat","omega","rho[1,2]","sigma","alpha","tau","sdpi","nu"), max_rows=13)
```
...
mod24gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod24gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
mod24cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod24cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod24cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
