...
This work focuses on multilevel modeling of isocratic HPLC retention data [@kamedulska_toward_2022,] [@kamedulska_statistical_2022,] [@wiczling_application_2021,] [@kubik_analysis_2018], incorporating structural similarity between analytes. Structural similarity was quantified using molecular fingerprints and the Tanimoto similarity metric calculated from SMILES representations.
...
The 2D structures of analytes, similarity matrix, log P and the number of functional groups were calculated using RDkit toolkit [@greg_landrum_2025_15773589]:
...
\end{aligned}
$$
where $R_i=(logkw_{i}, S_{1,i})$ is a vector of analyte-specific parameters, $logk_{i,j}$ corresponds to the above Neue equation, $MN$ is a matrix normal distribution, $\theta_R$ is a vector of typical values of $R_i$, $\beta_R$ is a vector of slopes between $R_i$ and $logP_i$, and $\pi_R$ is a vector of slopes between $R_i$ and functional group vector $Xi$. In turn, $\nu_{obs}$ and $\sigma$ is the normality parameters and scale of the residuals, $K$ and $\Omega$ are the scale matrices for the row and column covariace structures. $\Omega$ and $K$ were decomposed to:
...
where $LL'$ denotes correlation a correlation matrix, $\omega$ denotes standard deviation for between analyte variability, S is a similarity matrix and I is an Identify matrix. In this work the similarity matrix was simplified assuming that $S<0.5$ are 0 (essentially uncorrelated) and the values with $S \geq 0.5$ were scaled by alpha.
...
We compiled the model using cmdstanr and used optimization for initial testing.
...
We performed computations at the Academic Computer Center in Gdańsk, [Tryton Cluster](https://docs.task.gda.pl/kdm/zasoby-sprzetowe/tryton/). In this case:
...
3.  we loaded the output files using cmdstanr::as_cmdstan_fit: 
