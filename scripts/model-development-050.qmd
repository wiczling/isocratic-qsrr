...
```{r mcmc-intervals}
p1 <- mcmc_intervals(fit$draws(c("logkwHat","S1Hat","dlogkwHat","dS1Hat","beta[1]","beta[2]","S2Hat","alpha")))+ scale_y_discrete(labels = c( "logkwHat"= expression(theta[logkw]),
...
p2 <- mcmc_intervals(fit$draws(c("omega[1]","omega[2]", "rho[1,2]", "sigma","sdpi[1]","sdpi[2]","sdpi[3]","sdpi[4]"))) +
...
a =similarity_to_ltr_fun(cor_matrix)%>%
  rename(corr1 = similarity) %>% left_join(similarity_to_ltr_fun(cor_matrix_decorrelated)%>%
  rename(corr2 = similarity)) %>%
  mutate(dcorr = corr2^2-corr1^2) %>%
  left_join(similarity_ltri_rcdk_df) %>%
  filter(row!=col) %>%
  filter(col %in% row) 

# Compute quantiles per bin of similarity
quantile_df <- a %>%
  mutate(sim_bin = cut(similarity, breaks = 25)) %>%
  group_by(sim_bin) %>%
  summarise(
    similarity = median(similarity),
    p05 = quantile(dcorr, 0.05, na.rm = TRUE),
    p50 = mean(dcorr, na.rm = TRUE),
    p95 = quantile(dcorr, 0.95, na.rm = TRUE),
    .groups = "drop"
  )

# Plot with percentile overlays
p3 <- ggplot(a, aes(y = dcorr, x = similarity)) +
  geom_hex() +
  geom_line(data = quantile_df, aes(x = similarity, y = p05), color = "red", linetype = "dashed") +
  geom_line(data = quantile_df, aes(x = similarity, y = p50), color = "black", size = 1) +
  geom_line(data = quantile_df, aes(x = similarity, y = p95), color = "red", linetype = "dashed") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  theme_minimal() +
  labs(y = expression("Difference in variance explained"),
       x = "Tanimoto Similarity") +
...
figure_4A <- p1 + ggtitle("no individual data")
...
figure_5 <- df %>%
  ggplot(aes(x=maxsim, y = dvar))+
  geom_point(color = "steelblue")+
  geom_smooth(se = FALSE)+
  labs(y = "Variance explained (CV-based)", x="Maximum similarity within the hold-in set") 
figure_5
...
figure_4B <- (p1 + ggtitle("single observation"))
figure_4 <- figure_4A + figure_4B + plot_annotation(tag_levels = 'A')
```
...
dataobs = data %>%
  ...
  mutate(Analyte = str_replace_all(Analyte, "\n", " ")) %>%
              left_join(df_toc)%>%
  mutate(Analyte2 = paste0(Analyte, "/n", maxsim))
...
tab2 <- draws_gen_limited_df %>%
  ...
  mutate(logkind = sparam_ind.1-sparam_ind.2*(1+sS2Hat)*fi/(1+sS2Hat*fi)) %>%
  mutate(logkpop = sparam_pop.1-sparam_pop.2*(1+sS2Hat)*fi/(1+sS2Hat*fi))  %>%
  mutate(type = "single observation")
 ...
 fig_toc <- rbind(tab1,tab2)%>%
  ...
  ggdist::stat_lineribbon(aes(x = fi, y = logkpop), fill = "darkgray", color = "darkgray",   .width = c(.90), alpha = 1/2) +
  ggdist::stat_lineribbon(aes(x = fi, y = logkind), fill = "steelblue", color = "blue", .width = c(.90), alpha = 1/2) +
  geom_point(data=dataobs,aes(x=fi, y=logk)) +
...
ggsave(here::here("deliv","figures","manuscript","main", "parameter-estimates.png"),
       plot=figure_1, width = 20, height = 20, units = "cm", dpi = 300)
...
ggsave(here::here("deliv","figures","manuscript","main", "parameter-estimates-fg1.png"),
       plot=figure_2, width = 20, height = 20, units = "cm", dpi = 300)
...
ggsave(here::here("deliv","figures","manuscript","main", "correlations.png"),
       plot=figure_3, width = 20, height = 20, units = "cm", dpi = 300)
...
ggsave(here::here("deliv","figures","manuscript","main", "predictions.png"),
       plot=figure_4, width = 20, height = 20, units = "cm", dpi = 300)
...
ggsave(here::here("deliv","figures","manuscript","main", "var_explained.png"),
       plot=figure_5, width = 20, height = 20, units = "cm", dpi = 300)
...
ggsave(here::here("deliv","figures","manuscript","supplement", "parameter-estimates-fg2.png"),
       plot=figure_S2, width = 20, height = 20, units = "cm", dpi = 300)
