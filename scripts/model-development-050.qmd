...
This work focuses on multilevel modeling of isocratic HPLC retention data [@kamedulska_toward_2022,] [@kamedulska_statistical_2022,] [@wiczling_application_2021,] [@kubik_analysis_2018], incorporating structural similarity between analytes. Structural similarity, such as the Tanimoto similarity metric, can be easily calculated from SMILES representations and can help predict analyte retention when experimental data on structurally similar analytes is available.
...
RDKit was used to calculate the 2D structures of the analytes, the Tanimoto similarity matrix, log P, and the number of functional groups [@greg_landrum_2025_15773589]:
...
The logarithm of retention factor ($\small lok_{i,j}$) was modeled using the Neue model [@nikitas_20091737]:
...
& R_{i} \sim \text{MN}(\theta_R  + \beta_R \cdot log P_i +\pi1_{R} \cdot X_{1i}  +\pi2_{R} \cdot X_{2i}, K, \Omega) \\
...
where $\small LL'$ denotes  a correlation matrix, $\small \omega$ denotes standard deviation for between analyte variability, S is a similarity matrix and I is an Identify matrix. In this work the similarity matrix was simplified assuming S=0 for $\small S<0.5$ (analytes are essentially uncorrelated) and the values of S for $\small S \geq 0.5$ were scaled by alpha using the above equation.

A matrix normal distribution models random matrices, with parameters including a mean matrix and two covariance matrices, one for rows (analytes) and one for columns ($\small logkw$ and $\small S$ parameters). These covariance matrices, denoted here as $\small K$ and $\Omega$, capture the dependencies between rows and columns of the random matrix, respectively. The row covariance was related to similarity matrix approximating its structure.
...
param1<-draws_df_subset %>%
  ...
  mutate(type = if_else(functional_groups_names %in% dissociated_groups, "diss.", "non-diss."))
...
p_common <- param %>%
  ...
  ggplot(aes(y = functional_groups_name, x = dv, xmin = .lower, xmax = .upper, color= type)) +
  tidybayes::geom_pointinterval(size = 0.5, fatten = 1, linewidth = 0.4) +
  labs(x="Effect", y="Functional group", color = element_blank())+
  facet_wrap(. ~ param,
  ...
  geom_vline(xintercept = 0,color = "blue", linetype = "dotted")+
    theme(legend.position="bottom", legend.key.size = unit(0.3, 'cm'), legend.margin=margin(t=-10))
...
colnames(data_to_plot_param) <- c(expression('logkw'[i]),expression('S1'[i]),expression('logka'[i]), expression('logP'[i]))
...
data_to_plot_param <- cbind(param1,param2,param1-param2,datastruct$logPobs)
colnames(data_to_plot_param) <- c(expression('eta'[list(logkw,i)]),expression(eta[list(S1,i)]),expression(eta[list(logka,i)]),expression('logP'[i]))
...
p1 =similarity_to_ltr_fun(cor_matrix)%>%
  ...
  theme_minimal() + 
  labs(y=expression(atop(eta*" posterior", "correlations")),
       x = element_blank()) +
  theme(legend.position = "none")+
  ylim(-1, 1)
...
p2 =similarity_to_ltr_fun(cor_matrix_decorrelated)%>%
  ...
  labs(y=expression(atop("decorrelated " * eta, "posterior correlations")),
...
cond_cov <- conditional_covariance(U, V)
rho = cov2cor(cond_cov)[2,1]
sd  = sqrt(diag(cond_cov))[2]

var_base = 1.13^2*(1-0.88^2)
var_full = sd^2*(1-rho^2)
rel_var = (var_base-var_full)/var_base
...
Based on similarity and single experiment.
...
fun_plot_ind_pop <- function(draws, .analytes_idx=analytes_idx){
  ...
  ggdist::stat_lineribbon(aes(x = fi, y = logkind), fill = "steelblue", color = "blue", .width = c(.90), alpha = 1/2) +
  geom_point(data = filter(data,ID %in% .analytes_idx),aes(x=fi, y=logk), size = 0.5) +
  geom_point(data = filter(data[datastructcv$end,],ID %in% .analytes_idx),aes(x=fi, y=logk), color="red", size = 0.5) +  
  facet_wrap(~Analyte, ncol=2)+
  theme(strip.text = element_text(size = 3)) +
  labs(x="\u03C6", y="logk (individual and population)") +
  theme(legend.position = "none") +
...
figure_4B <- (p1 +theme(
  ...
  title = element_text(size = 6)) +
    ggtitle("similar analytes + single obs."))
figure_4A <- figure_4A +theme(
  title = element_text(size = 6)) 
...
# Limited data predictions: Without similarity.

Based on single experiment.
...
datastructcv$idxcv = idxcv;
datastructcv$nObscv = nObscv;
datastructcv$similarity_x= 0.5+0*datastructcv$similarity_x + diag(0.5,datastructcv$nAnalytes,datastructcv$nAnalytes,)
```

## Fitting the model

```{r stan-compile-cv4, eval = FALSE}
mod50cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

```{r dump-files-cv4, eval = FALSE}
write_stan_json(datastructcv, paste0(mod_model_dir,"/standata-nosim-limited.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(mod_model_dir,"/init-nosim-limited-1.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(mod_model_dir,"/init-nosim-limited-2.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(mod_model_dir,"/init-nosim-limited-3.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(mod_model_dir,"/init-nosim-limited-4.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(mod_model_dir,"/init-nosim-limited-5.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(mod_model_dir,"/init-nosim-limited-6.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(mod_model_dir,"/init-nosim-limited-7.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(mod_model_dir,"/init-nosim-limited-8.json"), always_decimal = FALSE)
```

```{r stan-load-cv4}
fitcvlimited <- cmdstanr::as_cmdstan_fit(c(
                                  paste0(mod_output_dir,"/",  model_name, "-nosim-limited-1.csv"),
                                  paste0(mod_output_dir,"/",  model_name, "-nosim-limited-2.csv"),
                                  paste0(mod_output_dir,"/",  model_name, "-nosim-limited-3.csv"),
                                  paste0(mod_output_dir,"/",  model_name, "-nosim-limited-4.csv"),
                                  paste0(mod_output_dir,"/",  model_name, "-nosim-limited-5.csv"),
                                  paste0(mod_output_dir,"/",  model_name, "-nosim-limited-6.csv"),
                                  paste0(mod_output_dir,"/",  model_name, "-nosim-limited-7.csv"),
                                  paste0(mod_output_dir,"/",  model_name, "-nosim-limited-8.csv")
                                ))
```

## Genereted quantitites

```{r generete-quntities-cv4, eval=FALSE}
mod50cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod50cvgq$generate_quantities(fitcvlimited,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
                                          output_basename = paste0(modelqg_name, "-nosim-limited"))
```

```{r eta-load-cv4}
x <- cmdstanr::read_cmdstan_csv(c(
                                  paste0(modgq_output_dir, "/",  modelqg_name, "-nosim-limited-1.csv"),
                                  paste0(modgq_output_dir, "/",  modelqg_name, "-nosim-limited-2.csv"),
                                  paste0(modgq_output_dir, "/",  modelqg_name, "-nosim-limited-3.csv"),
                                  paste0(modgq_output_dir, "/",  modelqg_name, "-nosim-limited-4.csv"),
                                  paste0(modgq_output_dir, "/",  modelqg_name, "-nosim-limited-5.csv"),
                                  paste0(modgq_output_dir, "/",  modelqg_name, "-nosim-limited-6.csv"),
                                  paste0(modgq_output_dir, "/",  modelqg_name, "-nosim-limited-7.csv"),
                                  paste0(modgq_output_dir, "/",  modelqg_name, "-nosim-limited-8.csv")
                                ))
draws_gen_nosim_limited_df <- as_draws_df(x$generated_quantities)
```

## Predictions
```{r ind-pop-predictions-cv4}
# 
fun_plot_ind_pop <- function(draws, .analytes_idx=analytes_idx){
  p1 <- draws %>%
  slice_sample(n=1000) %>%
...
p1<-fun_plot_ind_pop(draws_gen_nosim_limited_df,idx_analytes_out[1:10])
fun_plot_ind_pop(draws_gen_nosim_limited_df,idx_analytes_out[11:20])
fun_plot_ind_pop(draws_gen_nosim_limited_df,idx_analytes_out[21:30])
fun_plot_ind_pop(draws_gen_nosim_limited_df,idx_analytes_out[31:40])
fun_plot_ind_pop(draws_gen_nosim_limited_df,idx_analytes_out[41:50])
```

```{r compare}
# 
fun_plot_compare <- function(draws1, draws2, .analytes_idx=analytes_idx){
  tab1 <- draws1 %>%
  slice_sample(n=1000) %>%
  tidybayes::spread_draws(sparam_ind[i,..], sparam_pop[i,..], sS2Hat) %>%
  rename(ID=i) %>%
  ...
  p1 <- ggplot() +
  ggdist::stat_lineribbon(data = tab1, aes(x = fi, y = logkind), fill = "darkgray", color = "darkgray", .width = c(.90), alpha = 1/2) +
  ggdist::stat_lineribbon(data = tab2, aes(x = fi, y = logkind), fill = "steelblue", color = "blue", .width = c(.90), alpha = 1/2) +
  geom_point(data = filter(data,ID %in% .analytes_idx),aes(x=fi, y=logk), size = 0.5) +
  geom_point(data = filter(data[datastructcv$end,],ID %in% .analytes_idx),aes(x=fi, y=logk), color="red", size = 0.5) +
  facet_wrap(~Analyte, ncol=2)+
...
figure_S7<-fun_plot_compare(draws_gen_nosim_limited_df,draws_gen_limited_df,idx_analytes_out[1:10])

# 
#   tab1 <- draws_gen_nosim_limited_df %>%
#   slice_sample(n=1000) %>%
#   tidybayes::spread_draws(sparam_ind[i,..], sparam_pop[i,..], sS2Hat) %>%
#   rename(ID=i) %>%
#   left_join(analytes_names) %>%
#   filter(ID %in% idx_analytes_out) %>%
#   tidyr::expand_grid(fi = seq(0,1,0.1)) %>%
#   mutate(logkind = sparam_ind.1-sparam_ind.2*(1+sS2Hat)*fi/(1+sS2Hat*fi)) %>%
#   group_by(Analyte,ID, fi)%>%
#   summarise(mlogkind1 = var(logkind))
#   
...
#   mutate(logkind = sparam_ind.1-sparam_ind.2*(1+sS2Hat)*fi/(1+sS2Hat*fi)) %>%
#   group_by(Analyte,ID, fi)%>%
#   summarise(mlogkind2 = var(logkind))
#   
# tab=tab1%>%
#   left_join(tab2)%>%
#   mutate(ratio=(mlogkind1-mlogkind2)/mlogkind1)
#    
#  sim<-similarity_matrix_corrected
#  diag(sim)<-0
#  maxsim<-apply(sim, 1, max)
#  
#  tab$maxsim = round(maxsim[tab$ID],3)
#  tab %>%
#    ggplot(aes(x=maxsim, y = ratio, color = as.factor(fi)))+
#    geom_point(color = "steelblue", size = 0.5)+
#    geom_smooth(se = FALSE)+
#    labs(y = "Relative reduction in uncertainity", x="Maximum similarity within the hold-in set") 
```
...
ggsave(here::here("deliv","figures","manuscript","main", "correlations.png"),
       plot=figure_3, width = 8.46, height = 8.46*1, units = "cm", dpi = 300)
...
ggsave(here::here("deliv","figures","manuscript","supplement", "parameter-estimates-fg2.png"),
       plot=figure_S3, width = 8.46, height = 8.46*1.25, units = "cm", dpi = 300)
...
ggsave(here::here("deliv","figures","manuscript","supplement", "eta.png"),
       plot=figure_S6, width = 20, height = 20, units = "cm", dpi = 300)
ggsave(here::here("deliv","figures","manuscript","supplement", "eta.pdf"),
       plot=figure_S6, width = 20, height = 20, units = "cm", dpi = 300)

ggsave(here::here("deliv","figures","manuscript","supplement", "compare.png"),
       plot=figure_S7, width = 20, height = 20, units = "cm", dpi = 300)
ggsave(here::here("deliv","figures","manuscript","supplement", "compare.pdf"),
       plot=figure_S7, width = 20, height = 20, units = "cm", dpi = 300)
```
