...
mod4 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))

# mod4 <-  cmdstan_model(here::here(mod_model_dir, model_name_ext),
...
fit_opt <- mod4$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod4-opt",
  init = init,
  iter=3000
)
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod4-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod4-*.csv')
...
mod4gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod4gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
mod4cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))                
```
...
mod4cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod4cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
