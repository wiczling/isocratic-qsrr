...
Log P and functional groups calculated using open programs.
...
& R_{i} \sim \text{MST}(\nu, \theta_R  + \beta \cdot log P_i +\pi_R \cdot X_i, \Omega) \\
& \pi_{R,k} \sim N(0,\kappa_R)
\end{aligned}
$$
where $R_i=(logkw_{i}, S_{1,i})$ is a vector of subject-specific parameters, $logk_{i,j}$ corresponds to the above Neue equation, $MST$ is a multivariete student t distribution, $\theta_R$ is a vector of typical values of $R_i$, $\beta_R$ is a vector of slopes between $R_i$ and $logP_i$, and $\pi_R$ is a vector of slopes between $R_i$ and functional group vector $Xi$. In turn, $\sigma$ is the scale of the residuals, $\Omega$ is the scale matrix for unexplained between-analyte variabilities. $\Omega$ was decomposed to:
...
where $LL'$ denotes correlation a correlation matrix, $\omega$ denotes standard deviation for between analyte variability.
...
fit_opt$print(variables = c("logkwHat","S1Hat", "dlogkwHat","dS1Hat","beta","S2Hat","omega","rho[1,2]","sigma","sdpi"), max_rows=13)
