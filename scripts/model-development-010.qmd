...
nD <- nAnalytes_corr * (nAnalytes_corr - 1) %/% 2
point_mu_lower <- partial_correlation_subset[lower.tri(partial_correlation_subset, diag = FALSE)]
point_sd_lower <- ifelse(point_mu_lower == 0, 0.01, 0.2)
...
datastruct <- with(data,
                  list(nAnalytes=length(unique(data$ID)),
                       ...
                       logkobs=logk,
                       nD = nD,
                       point_mu_lower=point_mu_lower,
                       point_sd_lower = point_sd_lower,
                       run_estimation=1))
...
real lkj_corr_cholesky_point_lower_tri_lpdf(matrix cor_L, vector point_mu_lower, real point_scale_lower) {
...
transformed data {
  matrix[nAnalytes_corr, nAnalytes_corr] similarity_s;
  similarity_s = similarity_x[idx_corr,idx_corr];
  int lower_tri_d = nAnalytes_corr * (nAnalytes_corr - 1) %/% 2;
  vector[lower_tri_d] similarity_vector = lower_tri(similarity_s); 
}

parameters {
  real<lower=0> rhogp;
  cholesky_factor_corr[nAnalytes_corr] L_K;
}

model {
  target += lkj_corr_cholesky_point_lower_tri_lpdf(L_K | similarity_vector, rhogp);
  target += normal_lpdf(rhogp | 0, 0.1);
}

generated quantities {
 corr_matrix[nAnalytes_corr] rho = L_K * L_K';
}
")

idx_corelatted_subset <- idx_corelatted[group==17]
init_test <- function(){
  list(L_K = t(chol(similarity_matrix_corrected[idx_corelatted_subset,idx_corelatted_subset])),
         rhogp = 0.05+abs(rnorm(1,0,0.5)))
}

model <- cmdstan_model(stan_file = write_stan_file(stan_model_code))
data_list <- list(nAnalytes=length(unique(data$ID)),
                       nAnalytes_corr=length(idx_corelatted_subset),
                       idx_corr = idx_corelatted_subset,
                       similarity_x = similarity_matrix_corrected)
fit <- model$sample(data = data_list, chains=1, iter_sampling = 100, init=init_test)

fit$print(variables = c("rho[1,2]","rho[5,7]","rho[7,12]","rhogp"), max_rows=5)

mcmc_intervals(fit$draws("rho"))
```
...
mod10 <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```

We used optimization for initial testing:
```{r stan-optimize, eval = FALSE}
fit_opt <- mod10$optimize(
  data = datastruct,
  output_dir = mod_output_dir,
  output_basename = "mod10-opt",
  init = init,
  iter=3000
)
...
```{r stan-sample, eval = FALSE}
 ...
 fit$save_object(file = here(mod_output_dir,"mod10-loc.RDS"))
```
...
```{r stan-diagnose, eval = FALSE}
 ...
 str = paste0(cmdstan_path(), '/bin/diagnose  mod10-*.csv')
...
mod10gq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitgq  <- mod10gq$generate_quantities(fit,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 8,
...
mod10cv <- cmdstan_model(here::here(mod_model_dir, model_name_ext), stanc_options = list("O1"))
```
...
mod10cvgq <- cmdstan_model(here(modgq_model_dir,modelqg_name_ext))
fitcvgq  <- mod10cvgq$generate_quantities(fitcv,
                                          data = datastructcv,
                                          seed = 123,
                                          parallel_chains = 8,
                                          output_dir = modgq_output_dir,
