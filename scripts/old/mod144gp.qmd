...
stanfilescv_dir   <- here::here("model", "stan", model_name, "cv", "stanfiles")
...
if (!file.exists(stanfilescv_dir)) dir.create(stanfilescv_dir)
cv_dir   <- here::here("model", "stan", model_name, "cv")
if (!file.exists(cv_dir)) dir.create(cv_dir)
```
...
```{r clusterring}
distance = -2*log(simMat)
x<-hclust(as.dist(distance), method = "complete", members = NULL)
plot(x)
cut_avg <- cutree(x, k = 3)
```
...
data$logP_ACD[data$ID %in% 526] = mean(df$logP_ACD, na.rm = TRUE); # missing
...
#' select analytes with similarity index < 0.5
#' 
K_no_diag <- simMat
diag(K_no_diag)<-0
maxes = apply(K_no_diag,2,max)
ggplot(data.frame(x=1:1026, y=maxes), aes(x=x,y=y)) + 
  geom_point() + 
  geom_hline(yintercept = 0.6, color="red")

idx_corelatted = which(maxes>0.6)
idx_uncorelatted = which(maxes<=0.6)

simMat_adj <- simMat
simMat_adj[simMat_adj<=0.6]=0.6

K = 0.8*(simMat_adj[idx_corelatted,idx_corelatted]-0.6)/0.4
diag(K)<-1.00001
L_K=t(chol(K))

ggcorrplot(K[1:100,1:100],tl.cex=4)
ggcorrplot(simMat[idx_corelatted,idx_corelatted][1:100,1:100],tl.cex=4)
...
create_datastruct <- function(data, data_ACD, simMat, functional_groups) {
...
datastruct <- with(data,
                  list(nAnalytes=length(unique(data$ID)),
                       ...
                       idxA0 = idxA0,
                       idxB0 = idxB0,
                       simmat = simMat_adj[idx_corelatted,idx_corelatted],
                       run_estimation=1))
}


datastruct <- create_datastruct(data, data_ACD, simMat, functional_groups)
  
# initialize the values for each variable in each chain:
init <- function(){
  list(   logkwHat  = rnorm(1,4,2),
          S1Hat    = rnorm(1,4,1),
          dlogkwHat = rnorm(1,-1,0.125),
          dS1Hat   = rnorm(1,0,0.25),
          S2Hat = 2*exp(rnorm(1,0,0.125)),
          beta  = rnorm(2,c(0.7,0.5),c(0.125,0.5)),
          omega = c(1,1)*exp(rnorm(2, 0, 0.25)),
          rhoc =  t(chol(matrix(c(1,0.75,0.75,1), nrow=2))),
          alpha = 0.75,
          eta = cbind((init_aprox$logkw-mean(init_aprox$logkw))/sd(init_aprox$logkw),
                      (init_aprox$S1-mean(init_aprox$S1))/sd(init_aprox$S1)), #matrix(rep(0,nAnalytes*2), nrow=nAnalytes),
          pilogkw = rep(0,nK),
          piS1 = rep(0,nK),
...
fit_var <- mod$variational(
   data = datastruct,
   output_dir = stanfiles_dir,
   output_basename = paste0(model_name, "-var"),
   init = init,
   refresh = 100,
 )

fit_var$print(max_rows = 180)
```
...
```{r summary-prior, eval = FALSE, echo=FALSE}
 x<-as.data.frame(fit$summary(variables = c("logkwHat",
                              "S1Hat",
                              "dlogkwHat",
                              "dS1Hat",
                              "beta",
                              ...
                              "piS1",
                              "sdpi"), "mean", "sd")) %>%
  mutate(formula = paste0(variable, "~normal(", mean, ",", sd, ");"))
...
bayesplot::mcmc_trace(fit$draws(c("omega","rhoc[2,1]","sigma","alpha"))) 
```
...
mcmc_intervals(fit$draws(c("omega","rhoc[2,1]","sigma","alpha")))
```
...
x <- cmdstanr::read_cmdstan_csv(c(
                                  ...
                                  paste0(stanfiles_dir,"/",  model_name, "-gen-5.csv")
                                ))
...
results_ind <-draws_gen_df %>%
  slice_sample(n=1000) %>%
  tidybayes::spread_draws(slogkHat_ind[i]) %>% 
  rename(ID=i) %>%
  group_by(ID)%>%
  tidybayes::median_qi(logkpred = slogkHat_ind) %>%
  select(-ID)

results_pop <-draws_gen_df %>%
  slice_sample(n=1000) %>%
  tidybayes::spread_draws(slogkHat_pop[i]) %>% 
  rename(ID=i) %>%
  group_by(ID)%>%
  tidybayes::median_qi(logkpred = slogkHat_pop) %>%
  select(-ID)
...
analytes_idx = c(1:20); #c(1:9, 501,510, 626) (100:120)
...
initcv=init;
set.seed(1)
idx_analytes_out = sort(sample(unique(datastructcv$analyte),n_out))
ggcorrplot(simMat[idx_analytes_out,idx_analytes_out],tl.cex=4) +
...
```{r stancompile, eval = FALSE}
mod <- cmdstan_model(paste0(model_dir,"-cv.stan"), stanc_options = list("O1"))
```

We used optimization for initial testing:

```{r stanoptimize, eval = FALSE}
fitcv_opt <- mod$optimize(
  data = datastructcv,
  output_dir = stanfiles_dir,
  output_basename = paste0(model_name, "-opt"),
  init = initcv,
...
fitcv_opt$print(max_rows=100)
```

```{r optplots, eval=FALSE}
data$logkopt<-fitcv_opt$mle("logkx")

data %>%
  filter(ID %in% idx_analytes_out[50:62]) %>%
  ggplot(aes(x=fi, y=logk)) + 
  geom_point() + 
  geom_line(aes(x=fi, y=logkopt)) + 
  facet_wrap(~ID) +
...
data %>%
  filter(ID %in% idx_analytes_in[50:62]) %>%
  ggplot(aes(x=fi, y=logk)) + 
  geom_point() + 
  geom_line(aes(x=fi, y=logkopt)) + 
  facet_wrap(~ID) +
...
```{r dumpfiles, eval = FALSE}
write_stan_json(datastructcv, paste0(model_dir,"/cv/standata.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(model_dir,"/cv/init-1.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(model_dir,"/cv/init-2.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(model_dir,"/cv/init-3.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(model_dir,"/cv/init-4.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(model_dir,"/cv/init-5.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(model_dir,"/cv/init-6.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(model_dir,"/cv/init-7.json"), always_decimal = FALSE)
write_stan_json(initcv(), paste0(model_dir,"/cv/init-8.json"), always_decimal = FALSE)
```
...
fitcv <- cmdstanr::as_cmdstan_fit(c(
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-cv1.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-cv2.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-cv3.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-cv4.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-cv5.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-cv6.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-cv7.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-cv8.csv")
                                ))
...
```{r summary}
fitcv$print(max_rows = 180)
```

```{r generete-quntities, eval=FALSE}
model_gen <- cmdstan_model(paste0(stan_dir, "/", model_name, "-gen.stan"))
fit_gen  <- model_gen$generate_quantities(fitcv,
                                          data = datastruct,
                                          seed = 123,
                                          parallel_chains = 4,
                                          output_dir = stanfilescv_dir,
                                          output_basename = paste0(model_name, "-cv-gen"))
...
```{r etaload}
x <- cmdstanr::read_cmdstan_csv(c(
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-gen-1.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-gen-2.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-gen-3.csv"),
                                  paste0(stanfilescv_dir,"/",  model_name, "-cv-gen-4.csv")
                                ))
draws_gencv_df <- as_draws_df(x$generated_quantities)
...
```{r ind-predictions}
analytes_idx = idx_analytes_out[40:60]
...
values_to_select = c(0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1)
target.index = 0*values_to_select

for (i in 1:length(values_to_select)){
target.index[i] <- which(abs(maxes - values_to_select[i]) == min(abs(maxes - values_to_select[i])))[1]
}

idxcor<- sort(target.index)

datastructcor = datastruct;

datastructcor$idxcor = idxcor;
datastructcor$nObscor = length(idxcor);


K_no_diag[idxcor,idxcor]<-0
foo<-K_no_diag[idxcor,]
maxes_cor = apply(K_no_diag[idxcor,],1,max)
target.cor = 0 * maxes_cor
for (i in 1:length(maxes_cor)){
target.cor[i] <- which(abs(K_no_diag[idxcor[i],] - maxes_cor[i]) == min(abs(K_no_diag[idxcor[i],] - maxes_cor[i])))[1]
}


ggcorrplot(simMat[c(idxcor,target.cor),c(idxcor,target.cor)],tl.cex=4)
...
```{r generete-quntities, eval=FALSE}
model_cor <- cmdstan_model(paste0(stan_dir, "/", model_name, "-cor.stan"))
model_cor  <- model_cor$generate_quantities(fit,
                                          data = datastructcor,
                                          seed = 123,
                                          parallel_chains = 4,
                                          output_dir = stanfiles_dir,
                                          output_basename = paste0(model_name, "-cor"))
...
```{r etaload}
x <- cmdstanr::read_cmdstan_csv(c(
                                  paste0(stanfiles_dir,"/",  model_name, "-cor-1.csv"),
                                  paste0(stanfiles_dir,"/",  model_name, "-cor-2.csv"),
                                  paste0(stanfiles_dir,"/",  model_name, "-cor-4.csv"),
                                  paste0(stanfiles_dir,"/",  model_name, "-cor-5.csv")
                                ))
draws_cor_df <- as_draws_df(x$generated_quantities)
...
analytes_idx = target.index #target.cor
...
p <-draws_cor_df %>%
  slice_sample(n=1000) %>%
  tidybayes::spread_draws(simlogkHat_pop[a,f]) %>%
  rename(ID=a,logk=simlogkHat_pop) %>%
  left_join(analytes_names) %>%
  filter(ID %in% analytes_idx) %>%
  left_join(data.frame(f=c(1:11), fi = seq(0,1,by=0.1))) %>%
  ...
  geom_point(data = filter(data,ID %in% analytes_idx),aes(x=fi, y=logk)) + 
  facet_wrap(~Analyte)+
  labs(x="\u03C6", y="logk (population, given logk)") +
  theme(legend.position = "none") +
  coord_cartesian(xlim=c(0,1),ylim=c(-1,2))
