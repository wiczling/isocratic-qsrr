...
# ggcorrplot(simMat[401:500,401:500]2)
...
init <- function(){
  list(   logkwHat  = rnorm(1,2,2),
          ...
          etadlogkw = rnorm(nAnalytes,0,0.125),
          etadS1 = rnorm(nAnalytes,0,0.125), 
...
```{r aposterior-correlations}
mcmc_pairs(draws_gen_df, pars  = c("seta_ind[1,1]",
                                   "seta_ind[2,1]",
                                   "seta_ind[3,1]",
                                   "seta_ind[4,1]",
                                   "seta_ind[5,1]"), diag_fun = "dens")


etas<-draws_gen_df[,which(colnames(draws_gen_df) %in% grep("^seta_ind", names(draws_gen_df), value = TRUE))][1:1026]

corr_matrix = cor(etas)

rownames(simMat)<-1:1026
colnames(simMat)<-1:1026
rownames(corr_matrix)<-1:1026
colnames(corr_matrix)<-1:1026
ggcorrplot(simMat[001:100,001:100])
ggcorrplot(corr_matrix[001:100,001:100])
...
bayesplot::mcmc_trace(fit$draws(c("omega","rho[1,2]","kappa","sigma"))) 
...
mcmc_intervals(fit$draws(c("omega","rho[1,2]","sigma","kappa")))
```
...
p<-ggpairs(as.data.frame(data_to_plot_param), 
  ...
  #         aes(color = as.factor(cut_avg), alpha = 0.5),
