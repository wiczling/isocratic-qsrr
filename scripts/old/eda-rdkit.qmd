...
equations_ordered= read.csv(file = here(data_deliv_dir,"equations_ordered.csv"), header = TRUE)
...
eq <- data.frame(node = topo_order, parent = 0, crit = 0, stringsAsFactors = FALSE)

for (node in topo_order) {
  parent <- names(neighbors(g_directed, node, mode = "in"))
  
  if (length(parent) > 0) {
    eq$parent[eq$node == node] <- parent
    
    idx1 <- which(results_selected_crit$mol1_index == node & 
                  results_selected_crit$mol2_index == parent)
    idx2 <- which(results_selected_crit$mol2_index == node & 
                  results_selected_crit$mol1_index == parent)
    
    idx <- if (length(idx1) > 0) idx1 else idx2
    
    eq$crit[eq$node == node] <- results_selected_crit$crit[idx]
  }
}

all_nodes <- as.character(unique(analytes_names$ID))
missing_nodes <- setdiff(all_nodes, eq$node)
missing_df <- data.frame(node = missing_nodes, parent = "0", crit = 0, stringsAsFactors = FALSE)
equations_ordered <- rbind(missing_df, eq)

write.csv(equations_ordered, file = here(data_deliv_dir,"equations_ordered.csv"), row.names = FALSE)
```
...
plotting_i = function(i, .ilist) {
  data %>% 
    ...
    labs(
      title = paste(analytes_names$Analyte[.ilist$id1[i]], "(", .ilist$id1[i], ")", "\n",                                                      analytes_names$Analyte[.ilist$id2[i]], "(", .ilist$id2[i], ")"), 
      x = "\u03C6", 
...
plotting_i(1,comparison_table %>% filter(id1==574 & id2==952))
...
map(100+c(1:20), \(x) plotting_i(x, list1)) %>% wrap_plots(ncol = 4)
...
# fg_df_common1 <- fg_df_common1 %>% select(id1, id2, everything())
# fg_df_common2 <- fg_df_common2 %>% select(id1, id2, everything())
```

```{r prepare}

df_remove<- fg_df[comparison_table$id1,4:273]-fg_df_common1[,4:273]
df_add   <- fg_df[comparison_table$id2,4:273]-fg_df_common2[,4:273]
df_diff <- df_add-df_remove

df_diff<-df_diff %>%mutate(id1 = comparison_table$id1,
                      id2 = comparison_table$id2, .before = 1)

id1 <- as.character(df_diff$id1)
id2 <- as.character(df_diff$id2)

df1_colnames <- colnames(df_diff)  
new_columns <- as.data.frame(matrix(0, nrow = nrow(equations_ordered), ncol = length(df1_colnames)))
colnames(new_columns) <- df1_colnames
equations_fg_ordered <- cbind(equations_ordered, new_columns)

for (i in 1:nrow(equations_fg_ordered)) {
  node <- equations_fg_ordered$node[i]
  parent <- equations_fg_ordered$parent[i]
  
  if (parent == "0") {
    equations_fg_ordered[i,6:275] <- 0
  } else {
    # Check if parent == id1 and node == id2
    match1 <- df_diff[id1 == parent & id2 == node, ]
    if (nrow(match1) > 0) {
      equations_fg_ordered[i,4:275] <- match1
    }
    
    # Check if parent == id2 and node == id1
    match2 <- df_diff[id2 == parent & id1 == node, ]
    if (nrow(match2) > 0) {
      equations_fg_ordered[i,4:275] <- -match2
    }
  }
}

# equations_fg_ordered has NA values
equations_fg_ordered <- equations_fg_ordered %>%
  mutate(has_na = rowSums(is.na(.)) > 0) %>%
  arrange(!has_na) %>%
  mutate(id1 = if_else(has_na, 0, id1)) %>%
  mutate(id2 = if_else(has_na, 0, id2)) %>%
  mutate(crit = if_else(has_na, 0, crit)) %>%
  mutate(parent = if_else(has_na, "0", parent)) %>%
  select(-has_na)

write.csv(equations_fg_ordered, file = here(data_deliv_dir,"equations_fg_ordered.csv"), row.names = FALSE)
```
...
idx3_1 = which(equations_fg_ordered$node==idx1 & equations_fg_ordered$parent==idx2)
idx3_2 = which(equations_fg_ordered$parent==idx1 & equations_fg_ordered$node==idx2)
...
fg_df[c(idx1,idx2),]%>%select(where(~ !all(. == 0)))
df_diff[idx,]%>%select(where(~ !all(. == 0)))
equations_fg_ordered[idx3,]%>%select(where(~ !all(. == 0)))
...
map(1:min(20, nrow(listx)), \(x) plotting_i(x, listx)) %>% wrap_plots(ncol = 1)
